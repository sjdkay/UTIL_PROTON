void replay_production_coin (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "UTIL_PROTON/ROOTfiles/Analysis/ProtonLT/Proton_coin_replay_production_%d_%d.root";
  
  // Load global parameters
  gHcParms->Define("gen_run_number", "Run Number", RunNumber);
  //gHcParms->AddString("g_ctp_database_filename", "DBASE/COIN/standard_KaonLTCalib_Protons.database");
  ...
  // Define DEF-file+
  analyzer->SetOdefFile("UTIL_PROTON/config/DEF-files/coin_production.def");
  // Define cuts file
  analyzer->SetCutFile("UTIL_PROTON/config/DEF-files/coin_production_cuts.def");  // optional
  // File to record accounting information for cuts
  analyzer->SetSummaryFile(Form("UTIL_PROTON/config/REPORT_OUTPUT/Analysis/ProtonLT/summary_production_%d_%d.report", RunNumber, MaxEvent)); // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template	       
  analyzer->PrintReport("UTIL_PROTON/config/TEMPLATES/COIN/coin_production.template", Form("UTIL_PROTON/REPORT_OUTPUT/Analysis/ProtonLT/Proton_replay_coin_production_%d_%d.report", RunNumber, MaxEvent)); // optional}
}
