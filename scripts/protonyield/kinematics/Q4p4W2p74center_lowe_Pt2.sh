...
RunListFile="${UTILPATH}/scripts/protonyield/kinematics/Q4p4W2p74center_lowe_Pt2"
...
if [ ! -f "${UTILPATH}/scripts/protonyield/OUTPUT/Q4p4W2p74center_lowe_Pt2_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/protonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q4p4W2p74center_lowe_Pt2_Kaons\")"
elif [ ! -f "${UTILPATH}/scripts/protonyield/OUTPUT/Q4p4W2p74center_lowe_Pt2_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/protonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q4p4W2p74center_lowe_Pt2_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/scripts/protonyield/OUTPUT/Q4p4W2p74center_lowe_Pt2_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
