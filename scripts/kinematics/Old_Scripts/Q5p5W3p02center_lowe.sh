...
if [ ! -f "${UTILPATH}/scripts/protonyield/OUTPUT/Q5p5W3p02center_lowe_Protons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/protonyield/PlotProtonPhysics.C(\"${KINFILE}\", \"Q5p5W3p02center_lowe_Protons\")"
elif [ ! -f "${UTILPATH}/scripts/protonyield/OUTPUT/Q5p5W3p02center_lowe_Protons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/protonyield/PlotProtonPhysics.C(\"${KINFILE}\", \"Q5p5W3p02center_lowe_Protons\")"
else echo "Proton plots already found in - ${UTILPATH}/scripts/protonyield/OUTPUT/Q5p5W3p02center_lowe_Protons.root and .pdf - Plotting macro skipped"
fi
exit 0
