...
if [ ! -f "${UTILPATH}/scripts/protonyield/OUTPUT/Q4p4W2p74center_lowe_Pt1_Protons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/protonyield/PlotProtonPhysics.C(\"${KINFILE}\", \"Q4p4W2p74center_lowe_Pt1_Protons\")"
elif [ ! -f "${UTILPATH}/scripts/protonyield/OUTPUT/Q4p4W2p74center_lowe_Pt1_Protons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/protonyield/PlotProtonPhysics.C(\"${KINFILE}\", \"Q4p4W2p74center_lowe_Pt1_Protons\")"
else echo "Proton plots already found in - ${UTILPATH}/scripts/protonyield/OUTPUT/Q4p4W2p74center_lowe_Pt1_Protons.root and .pdf - Plotting macro skipped"
fi
exit 0
