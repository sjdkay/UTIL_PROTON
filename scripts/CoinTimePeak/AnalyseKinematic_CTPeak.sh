...
while IFS='' read -r line || [[ -n "$line" ]]; do
    runNum=$line
    if [ ! -f "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_-1_CTPeak_Data.root" ]; then
...
if [ $TestingVar == 1 ]; then
	...
	while IFS='' read -r line || [[ -n "$line" ]]; do
	    runNum=$line
	    if [ -f "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_-1_CTPeak_Data.root" ]; then
		rm "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_-1_CTPeak_Data.root"
	    fi
	...
	if [[ $prompt2 == "y" || $prompt2 == "Y" || $prompt2 == "yes" || $prompt2 == "Yes" ]]; then
		...
		runNum=$line
		if [ ! -f "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_-1_CTPeak_Data.root" ]; then
		    python3 $UTILPATH/scripts/CoinTimePeak/src/CoinTimePeak.py "Proton_coin_replay_production" ${runNum} "-1" 
		fi
	    done < "$RunListFile"
