...
while IFS='' read -r line || [[ -n "$line" ]]; do
    runNum=$line
    if [ ! -f "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_-1_CTPeak_Data_HeepCoin.root" ]; then
...
if [ $TestingVar == 1 ]; then
	...
	while IFS='' read -r line || [[ -n "$line" ]]; do
	    runNum=$line
	    if [ -f "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_-1_CTPeak_Data_HeepCoin.root" ]; then
		rm "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_-1_CTPeak_Data_HeepCoin.root"
	    fi
	    if [ -f "${UTILPATH}/ROOTfiles/Analysis/ProtonLT/Proton_coin_replay_production_${runNum}_-1.root" ]; then
		rm "${UTILPATH}/ROOTfiles/Analysis/ProtonLT/Proton_coin_replay_production_${runNum}_-1.root"
	    fi
	...
	if [[ $prompt2 == "y" || $prompt2 == "Y" || $prompt2 == "yes" || $prompt2 == "Yes" ]]; then
		...
		runNum=$line
		if [ ! -f "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_-1_CTPeak_Data_HeepCoin.root" ]; then
		    python3 $UTILPATH/scripts/CoinTimePeak/src/CoinTimePeak_HeepCoin.py "Proton_coin_replay_production" ${runNum} "-1" 
		fi
	    done < "$RunListFile"
...
if [ $TestingVar == 1 ]; then
    if [ -f "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${KINEMATIC}_Output.csv" ]; then
	rm "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${KINEMATIC}_Output.csv"
    else touch "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${KINEMATIC}_Output.csv"
    fi
    while IFS='' read -r line || [[ -n "$line" ]]; do
	runNum=$line
	OutputFile="${UTILPATH}/OUTPUT/Analysis/ProtonLT/${runNum}_Out_tmp"
	...
	echo ${Data} >> "${UTILPATH}/OUTPUT/Analysis/ProtonLT/${KINEMATIC}_Output.csv"
	rm ${OutputFile}
	sleep 1
    done < "$RunListFile"
fi
